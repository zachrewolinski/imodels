...
from imodels.tree.rf_plus.rf_plus_prediction_models.aloocv_regression import AloGLMRegressor, AloElasticNetRegressor, AloRidgeRegressor, AloLassoRegressor
from imodels.tree.rf_plus.rf_plus_prediction_models.aloocv_classification import AloGLMClassifier, AloLogisticRidgeClassifier, AloLogisticLassoClassifier, AloLogisticElasticNetClassifier, AloSVCRidgeClassifier

#XGBoost 
from xgboost import XGBRegressor, XGBClassifier

# Feature Importance and util imports
...
class _RandomForestPlus(BaseEstimator):
    ...
    def __init__(self, rf_model=None, prediction_model=None, include_raw=True,
                 ...
                 normalize=False, fit_on = "auto",verbose = True):
        ...
        if prediction_model is None:
            if self._task == "regression":
                prediction_model =  AloRidgeRegressor(cv = cv) #GlmNetElasticNetRegressorPPM(cv = cv) #
            elif self._task == "classification":
                prediction_model = AloLogisticRidgeClassifier(cv = cv) #GLMLogisticLassoNetPPM(cv = cv) #GLMLogisticElasticNetPPM(cv = cv) #SVCRidgePPM(cv=cv)#
        ...
        self.cv = cv
        self.initial_prediction_model = prediction_model
    ...
    def fit(self, X, y, sample_weight=None, n_jobs = None,rescale_mode = "max",**kwargs):
        """
        ...
        **kwargs:
        ...
        # if isinstance(self.initial_prediction_model, GlmClassifierPPM):
        ...
        def parallel_fit_helper(tree_model,i):
            if self.verbose:
                pprint.pprint(f"Fitting batch {i}")
            ...
            #blocked_data = transformer.fit_transform(X_array, center=self.center, normalize=self.normalize)
            prediction_model = copy.deepcopy(self.initial_prediction_model)
            # do sample split
            ...
            if self.fit_on == "inbag":
                train_blocked, train_y = in_bag_blocked_data, y_in_bag
            elif self.fit_on == "oob":
                train_blocked, train_y = oob_blocked_data, y_oob
            else:
                train_blocked, train_y = blocked_data, y

            if in_bag_blocked_data.get_all_data().shape[1] != 0:
                prediction_model.fit(in_bag_blocked_data.get_all_data(), y_in_bag, **kwargs)
            
            
            return copy.deepcopy(prediction_model), copy.deepcopy(transformer), tree_model.random_state
    ...
    def predict_proba(self, X,n_jobs = 1):
        ...
        # blocked_data = self.transformers_[0].transform(X_array, center=self.center, normalize=self.normalize)
        # print(self.estimators_[0].predict_proba(blocked_data.get_all_data())[:,1].shape)
...
if __name__ == "__main__":
    ...
    # rf_plus_sklearn = RandomForestPlusRegressor(rf_model=copy.deepcopy(rf))
    ...
    X, y,f = imodels.get_clean_dataset("csi_pecarn")
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3)
    
    rf = RandomForestClassifier(n_estimators=5,min_samples_leaf=5,max_features="sqrt",class_weight="balanced")
    rf.fit(X_train, y_train)
    ...
    # rf_plus_nodrop = RandomForestPlusClassifier(rf_model=copy.deepcopy(rf),drop_features=False)
    # rf_plus_nodrop.fit(X_train, y_train,n_jobs=-1)

    #rf_plus_svm = RandomForestPlusClassifier(rf_model=copy.deepcopy(rf),prediction_model=AloSVCRidgeClassifier())
    #rf_plus_svm.fit(X_train, y_train,n_jobs=-1)

    rf_plus_generic = RandomForestPlusClassifier(rf_model=copy.deepcopy(rf),
                                                 prediction_model=MLPClassifier(hidden_layer_sizes=(128,128,),
                                                                                activation="relu",alpha=0.2))
    rf_plus_generic.fit(X_train, y_train,n_jobs=-1)

    MLP= MLPClassifier(hidden_layer_sizes=(64,64,),activation="relu",alpha=6.8,max_iter=1000)
    ...
    for metric in prob_metrics:
        ...
        pprint.pprint(f"RF+ {metric.__name__}: {metric(y_test,rf_plus.predict_proba(X_test)[:,1])}")
        #pprint.pprint(f"RF+ SVM {metric.__name__}: {metric(y_test,rf_plus_svm.predict_proba(X_test))}")
        pprint.pprint(f"RF+ XGB {metric.__name__}: {metric(y_test,rf_plus_generic.predict_proba(X_test)[:,1])}")
        pprint.pprint(f"MLP {metric.__name__}: {metric(y_test,MLP.predict_proba(X_test)[:,1])}")
    ...
    for metric in class_metrics:
        ...
        pprint.pprint(f"RF+ XGB {metric.__name__}: {metric(y_test,rf_plus_generic.predict(X_test))}")
        
        pprint.pprint(f"MLP {metric.__name__}: {metric(y_test,MLP.predict(X_test))}")
