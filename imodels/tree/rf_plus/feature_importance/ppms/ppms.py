...
class MDIPlusGenericRegressorPPM(ABC):
    ...
    def predict_partial_subtract_intercept(self, blocked_data, mode, l2norm, sign, njobs = 1):
        n_blocks = blocked_data.n_blocks
        def predict_wrapper(k):
            start_pred_k = time.time()
            predict_k = self.predict_partial_k_subtract_intercept(blocked_data, k, mode, l2norm, sign=sign)
...
class MDIPlusGenericClassifierPPM(ABC):
    ...
    def predict_partial_subtract_intercept(self, blocked_data, mode, l2norm, sign, sigmoid=False, zero_values=None, njobs = 1):
        n_blocks = blocked_data.n_blocks
        partial_preds = {}
        for k in range(n_blocks):
            if zero_values is not None:
