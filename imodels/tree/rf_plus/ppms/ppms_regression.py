...
class ElasticNetRegressorPPM(GlmRegressorPPM):
    """
    PPM class for Elastic
    """

    def __init__(self, cv="loo", n_alphas=100, l1_ratio = 0.5, standardize = True, **kwargs):
        if cv == "loo":
            n_splits = 0
        else:
            n_splits = cv
...
class RidgeRegressorPPM(GlmRegressorPPM):
    """
    Ppm class for regression that uses ridge as the GLM estimator.
    """
    def __init__(self, cv="loo", n_alphas=100, standardize = True, **kwargs):
        if cv == "loo":
            n_splits = 0
        else:
            n_splits = cv
...
class LassoRegressorPPM(GlmRegressorPPM):
    """
...
if __name__ == "__main__":
    ...
    model = ElasticNetRegressorPPM(l1_ratio=0.5, cv=3)
